...
exports.getMyProfile = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId)
      ...
      .populate("pendingManager", "name email");
...
exports.getMyTeam = async (req, res) => {
  try {
    ...
    const teamWithMetrics = await Promise.all(team.map(async (u) => {
      const uObj = u.toObject();
      ...
      // Fetch IDP Stats
      try {
        const idps = await IDP.find({ employee: u._id });
        const sevenDaysAgo = new Date();
        sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);

        // Total Stats
        uObj.totalIDPs = idps.length;
        uObj.completedGoals = idps.reduce((total, idp) => {
          const completedResources = idp.recommendedResources
            ? idp.recommendedResources.filter(r => r.status === 'completed').length
            : 0;
          return total + completedResources;
        }, 0);

        // Weekly Stats (Last 7 Days)
        uObj.weeklyCreatedIDPs = idps.filter(idp => new Date(idp.createdAt) > sevenDaysAgo).length;

        uObj.weeklyCompletedGoals = idps.reduce((total, idp) => {
          const recentCompletedResources = idp.recommendedResources
            ? idp.recommendedResources.filter(r =>
              r.status === 'completed' &&
              r.completedAt &&
              new Date(r.completedAt) > sevenDaysAgo
            ).length
            : 0;
          return total + recentCompletedResources;
        }, 0);

      } catch (err) {
        console.error(`Error fetching IDPs for user ${u._id}:`, err);
        uObj.totalIDPs = 0;
        uObj.completedGoals = 0;
        uObj.weeklyCreatedIDPs = 0;
        uObj.weeklyCompletedGoals = 0;
      }
...
exports.joinTeam = async (req, res) => {
  try {
    ...
    // Set pending manager instead of direct assignment
    await User.findByIdAndUpdate(userId, { pendingManager: manager._id });

    res.json({
      message: `Request sent to ${manager.name}. Awaiting approval.`,
      pendingManager: {
        id: manager._id,
        name: manager.name,
        email: manager.email
 ...
 * GET /api/user/team-requests
 */
exports.getPendingTeamRequests = async (req, res) => {
  try {
    const requests = await User.find({ pendingManager: req.user.id })
      .select('name email avatar')
      .lean();

    // Process avatar similarly if needed, for now just basic info
    res.json(requests);
  } catch (error) {
 ...
 * PUT /api/user/team-requests/:userId/approve
 */
exports.approveTeamMember = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ _id: userId, pendingManager: managerId });
    if (!user) {
      return res.status(404).json({ message: "Request not found" });
    }

    user.manager = managerId;
    user.pendingManager = null;
    await user.save();

    res.json({ message: `Approved ${user.name} to join your team` });
  } catch (error) {
    console.error("Approve Team Member Error:", error);
    res.status(500).json({ message: "Server error" });
  }
};

/**
 * REJECT TEAM MEMBER (Manager Only)
 * PUT /api/user/team-requests/:userId/reject
 */
exports.rejectTeamMember = async (req, res) => {
  try {
    ...
    res.json({ message: `Rejected request from ${user.name}` });
  } catch (error) {
    console.error("Reject Team Member Error:", error);
    res.status(500).json({ message: "Server error" });
  }
};
