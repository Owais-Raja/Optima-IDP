...
// Dashboard Widgets
// -------------------------------------------------------------------------------------------------
// Collection of reusable UI widgets for dashboards.
// Includes:
// - WelcomeBanner: Personalized greeting and status.
// - StatCard: Display for key metrics (KPIs).
// - ActivityFeedItem: Individual item for recent activity lists.
// - NotificationItem: Individual item for notification lists.
// =================================================================================================
...
// ======================= Stat Card Component =======================
export const StatCard = ({ label, value, subtext, icon: Icon, color = 'purple', trend, onClick }) => {
    const colors = {
    ...
    return (
        <div
            onClick={onClick}
            className={`bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-xl p-5 hover:border-slate-700 transition-all group hover:transform hover:scale-[1.02] duration-300 ${onClick ? 'cursor-pointer hover:shadow-lg hover:shadow-purple-500/10' : ''}`}
        >
